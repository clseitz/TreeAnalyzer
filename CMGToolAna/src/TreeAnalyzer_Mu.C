...
void TreeAnalyzer(TString list, TString outname,bool useW=true){
	...
	const char * CutList[CutNumb] = {"noCut",
					 "== 1 Mu", "6Jet","HT> 500","bjet>1","DelW>1","ST > 250"
	                                };
	//top
	...
	TH1F *h3bJetpT[CutNumb];
        ...
        for(int cj = 0; cj < CutNumb; cj++)
	  {
	    ...
	    hdPhi[cj]->Sumw2();
	    hdPhiWLep[cj] = new TH1F("dPhiWLep_"+nCut,"dPhiWLep "+cutName,64,0,3.2);
	    hdPhiWLep[cj]->Sumw2();
...
//1;//fw; //lumi calcualtion done in runAnalyzer.py (fb and pb)
	  ...
	  for(int ilep=0;ilep<nLep;ilep++)
		...
		if( dummy.Pt() > goodLepPt &&  LepGood_tightId[ilep] ){ //TODO: need to adjust isolation
		  ...
		  if(abs(LepGood_pdgId[ilep]) == 13 && LepGood_relIso03[ilep] < 0.12){
	  ...
	  if (nJetGood < 2) continue;
                ...
                if (DelPhiMetMu > TMath::Pi()) DelPhiMetMu-=TMath::Pi();
                ...
                float goodMu_py = goodMu[0].Pt()*TMath::Sin(met_phi);
                float goodMu_px = goodMu[0].Pt()*TMath::Cos(met_phi);
                float Wphi = TMath::ATan( (met_py + goodMu_py) / (met_px + goodMu_px ) );
                float DelPhiWlep = fabs(Wphi - goodMu[0].Phi());
                //cout<<DelPhiWlep<<endl;
	  ...
	  hdPhiWLep[1]->Fill(DelPhiWlep,EvWeight);
	  ...
	  hdPhiWLep[2]->Fill(DelPhiWlep,EvWeight);
	  ...
	  hdPhiWLep[3]->Fill(DelPhiWlep,EvWeight);
	  ...
	  hnJet[6]->Fill(nJetGood,EvWeight);
	  hHT[6]->Fill(HT40,EvWeight);
	  hST[6]->Fill(ST,EvWeight);
	  hdPhiWLep[6]->Fill(DelPhiWlep,EvWeight);
